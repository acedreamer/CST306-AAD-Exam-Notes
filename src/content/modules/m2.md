---
title: "Graph Algorithms & AVL Trees"
moduleNumber: 2
order: 2
---
...
*   **Strategy:** Visits vertices layer by layer. It starts at a source vertex $u$, visits all its neighbors, then all neighbors of neighbors, and so on.
*   **Data Structure:** Uses a **Queue**.
*   **Algorithm BFS(G, u):**
    1.  Initialize all nodes as unvisited.
    2.  Mark starting vertex $u$ as visited and enqueue it.
...
*   **Strategy:** Dives as deep as possible into the graph before backtracking.
*   **Data Structure:** Uses a **Stack** (implicitly via recursion).
*   **Algorithm DFS(G, u):**
    1.  Mark vertex $u$ as visited.
    2.  For each adjacent vertex $v$ of $u$:
        a. If $v$ is not visited, call **DFS(G, v)** recursively.
*   **Edge Classification (During DFS):**
    *   **Tree Edge:** Edges in the DFS forest (actually used to visit a new node).
    *   **Back Edge:** Edge connecting a node to an ancestor in the DFS tree (indicates a cycle).
    *   **Forward Edge:** Edge connecting a node to a descendant that is already visited.
    *   **Cross Edge:** Edge connecting two nodes that have no ancestor/descendant relationship.
...
**Example: Perform BFS and DFS on a Graph**
Consider vertices $\{A, B, C, D\}$ with edges $(A,B), (A,C), (B,D)$.

**BFS Trace (Start at A):**
1.  **Queue:** $[A]$. Visited: $\{A\}$.
2.  Dequeue $A$. Neighbors: $B, C$.
3.  **Queue:** $[B, C]$. Visited: $\{A, B, C\}$.
4.  Dequeue $B$. Neighbor: $D$.
5.  **Queue:** $[C, D]$. Visited: $\{A, B, C, D\}$.
6.  Dequeue $C$. No unvisited neighbors.
7.  Dequeue $D$. No unvisited neighbors.
8.  **BFS Order:** $A \to B \to C \to D$.

**DFS Trace (Start at A):**
1.  Visit $A$. Neighbors: $B, C$. Pick $B$ (alphabetical).
2.  Visit $B$. Neighbor: $D$.
3.  Visit $D$. No unvisited neighbors. Backtrack to $B$.
4.  $B$ has no more unvisited neighbors. Backtrack to $A$.
5.  Visit $C$ (next neighbor of $A$).
6.  **DFS Order:** $A \to B \to D \to C$.
...
**Construct an AVL tree for: 3, 2, 1**
1.  **Insert 3:** BF=0. (Root)
2.  **Insert 2:** 2 goes to left of 3. BF(3)=1, BF(2)=0. Balanced.
3.  **Insert 1:** 1 goes to left of 2.
    *   BF(3) = 2 (Unbalanced!)
    *   BF(2) = 1
    *   Path of imbalance: Left-Left (LL).
4.  **Perform RR Rotation (Right rotate at 3):**
    *   2 becomes the new root.
    *   3 becomes the right child of 2.
    *   1 remains the left child of 2.
5.  **Final Tree:** Root(2), Left(1), Right(3). All BFs = 0.
...
**Kosaraju's Trace on: A → B, B → C, C → A, B → D**
1.  **Pass 1 (DFS on G):** Visit A, then B, then C. Backtrack C, B. From B, visit D. Backtrack D, A.
    *   Stack (order of finish): $[D, C, B, A]$ (A finished last).
2.  **Pass 2 (Reverse G):** Edges become B → A, C → B, A → C, D → B.
3.  **Process Stack:**
    *   Pop A: DFS(A) on reversed graph visits A, C, B. **SCC 1 = {A, B, C}**.
    *   Pop B, C: Already visited.
    *   Pop D: DFS(D) visits only D. **SCC 2 = {D}**.
